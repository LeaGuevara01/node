...
function ExampleComponent() {
  ...
  return <button onClick={handleClick}>Ejemplo con Logging</button>;
}
...
function CrudExample() {
  ...
  return (
    <div>
      <button onClick={() => createMaquinaria({ nombre: 'Tractor Nuevo' })}>Crear</button>
    </div>
  );
}
...
function PerformanceExample() {
  ...
  return <button onClick={heavyOperation}>Operación Pesada</button>;
}
...
function NavigationExample() {
  ...
  return (
    <nav>
      <button onClick={() => handleNavigation('/maquinarias')}>Ir a Maquinarias</button>
    </nav>
  );
}
...
function ContextExample({ userId, role }) {
  ...
  return <button onClick={handleAction}>Acción con Contexto</button>;
}
