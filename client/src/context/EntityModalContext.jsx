...
export function EntityModalProvider({ children, token }) {
  ...
  const openEntityModal = useCallback(
    async ({ entity, mode = 'create', item = null, props = {} }) => {
      ...
      {modalState.open && LoadedComponent && (
        <Suspense
          fallback={
            <div className="fixed inset-0 flex items-center justify-center bg-black/30 z-50">
              <div className="bg-white px-4 py-3 rounded shadow text-sm text-gray-600">
                Cargando…
              </div>
            </div>
          }
        >
          <LoadedComponent
            ...
            {...(modalState.entity === 'maquinarias'
              ? {
                  maquinaria: modalState.item || {},
                  onCreate: modalState.props.onCreate,
                  onUpdate: modalState.props.onUpdate,
                  onDelete: modalState.props.onDelete,
                }
              : {})}
            {...(modalState.entity === 'repuestos'
              ? {
                  repuesto: modalState.item || {},
                  onCreate: modalState.props.onCreate,
                  onUpdate: modalState.props.onUpdate,
                  onDelete: modalState.props.onDelete,
                }
              : {})}
            {...(modalState.entity === 'proveedores'
              ? {
                  proveedor: modalState.item || {},
                  onCreate: modalState.props.onCreate,
                  onUpdate: modalState.props.onUpdate,
                  onDelete: modalState.props.onDelete,
                }
              : {})}
            {...(modalState.entity === 'reparaciones'
              ? {
                  item: modalState.item || {},
                  onSave: modalState.props.onSave,
                  onDelete: modalState.props.onDelete,
                  users: modalState.props.users,
                  maquinarias: modalState.props.maquinarias,
                  repuestos: modalState.props.repuestos,
                }
              : {})}
