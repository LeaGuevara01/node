...
const useSuggestions = (data = [], fieldConfig = {}) => {
  ...
  const generateSuggestions = useCallback((fieldKey, inputValue = '') => {
  ...
  const handleSuggestionClick = useCallback((fieldKey, suggestion, onFilterChange) => {
  ...
  const handleInputChange = useCallback((fieldKey, value, onFilterChange) => {
  ...
  const getInputValue = useCallback((fieldKey, fallbackValue = '') => {
    return suggestionInputs[fieldKey] !== undefined 
      ? suggestionInputs[fieldKey] 
      : fallbackValue;
  }, [suggestionInputs]);

  // Verificar si un campo tiene sugerencias activas
  const hasSuggestions = useCallback((fieldKey) => {
    return activeSuggestions[fieldKey] === true;
  }, [activeSuggestions]);

  // Obtener sugerencias para un campo específico
  const getSuggestions = useCallback((fieldKey, inputValue) => {
  ...
  marca: (dataFieldName = 'marca') => ({
    getSuggestions: (inputValue, data) => {
      const marcas = [...new Set(
        data.map(item => item[dataFieldName])
          .filter(Boolean)
  ...
  proveedor: (dataFieldName = 'proveedor') => ({
    getSuggestions: (inputValue, data) => {
      const proveedores = [...new Set(
        data.map(item => item[dataFieldName] || item.nombreProveedor)
          .filter(Boolean)
  ...
  localidad: (dataFieldName = 'localidad') => ({
    getSuggestions: (inputValue, data) => {
      const localidades = [...new Set(
        data.map(item => item[dataFieldName] || item.ciudad)
          .filter(Boolean)
