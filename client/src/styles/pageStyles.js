  ...
  return (
    <div className={`${PAGE_STYLES.pageContainer} ${className}`}>
      <div className={PAGE_STYLES.mainContent}>
        <div className={PAGE_STYLES.contentWrapper}>{children}</div>
      </div>
    </div>
  );
}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          {showBackButton && onBack && (
            <button onClick={onBack} className={PAGE_STYLES.backButton} aria-label="Volver">
              <ChevronLeft size={20} />
              <span>Volver</span>
            </button>
          )}
          <div>
            ...
            {subtitle && <p className={PAGE_STYLES.pageSubtitle}>{subtitle}</p>}
          </div>
        </div>
 ...
 */
export function ContentSection({ title, children, className = '', padding = 'p-6' }) {
  return (
    <section
      className={`bg-white rounded-xl shadow-sm border border-gray-200 ${padding} mb-6 ${className}`}
    >
 ...
 */
export function ResponsiveGrid({ children, columns = 'default', gap = 'gap-4', className = '' }) {
  const columnStyles = {
  ...
  return <div className={`grid ${columnStyles[columns]} ${gap} ${className}`}>{children}</div>;
}
