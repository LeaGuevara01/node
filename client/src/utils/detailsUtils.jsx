...
/**
 * Manejo de carga de archivos/imágenes
 */
export const handleFileUpload = async (file, entityId, entityType, token) => {
  ...
  const response = await fetch(
    `${import.meta.env.VITE_API_URL || 'http://localhost:4000/api'}/${entityType}/${entityId}/upload`,
    {
      method: 'POST',
      headers: {
...
/**
 * Renderizar campo con icono
 */
import { Link } from 'react-router-dom';

export const renderFieldWithIcon = (label, value, icon, link = null) => {
    ...
    <div className="mt-1 p-3 bg-gray-50 border border-gray-200 rounded-md flex items-center">
      ...
      {link ? (
        String(link || '').startsWith('http') ? (
          <a
            href={link}
            className="text-brand-700 hover:text-brand-900 ml-2"
            target="_blank"
          ...
          <Link to={link} className="text-brand-700 hover:text-brand-900 ml-2">
            {value}
          </Link>
        )
      ) : (
...
/**
 * Renderizar campo simple
 */
export const renderField = (label, value, className = 'sm:col-span-1') => {
  if (!value) return null;

  return (
    <div className={className}>
      <label className="block text-sm font-medium text-gray-700 mb-2">{label}</label>
      <div className="mt-1 p-3 bg-gray-50 border border-gray-200 rounded-md">{value}</div>
    </div>
  );
};
