...
function ReparacionesPage({ token, role, onLogout, onCreated }) {
      ...
      <>
        <div className={LIST_STYLES.itemHeader}>
          <div className="flex items-start gap-2 min-w-0">
            <h3 className={`${LIST_STYLES.itemTitle} truncate`} title={titulo}>
              {titulo}
            </h3>
            {reparacion.estado && (
              <span
                className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getEstadoBadge(reparacion.estado)}`}
              >
                {reparacion.estado}
              </span>
            )}
        ...
        <div className="mt-1 text-sm sm:text-base text-gray-600 flex flex-wrap gap-x-3 gap-y-1">
          {reparacion.maquinaria && (
            <span className="inline-flex items-center gap-1" title="Maquinaria">
              <svg className={ICON_STYLES.xs} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
              ...
              <span className="truncate max-w-[10rem] sm:max-w-[16rem]">
                {[reparacion.maquinaria?.nombre, reparacion.maquinaria?.modelo]
                  .filter(Boolean)
                  .join(' ')}
              </span>
            </span>
            ...
            <span className="inline-flex items-center gap-1" title="Repuestos usados">
              <svg className={ICON_STYLES.xs} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M9.75 3l.75 4.5L6 9l4.5.75L9.75 15l2.25-3.75L14.25 15l-.75-4.5L18 9l-4.5-.75L14.25 3 12 6.75 9.75 3z"
                />
              </svg>
        ...
        <div className={`${LIST_STYLES.itemTagsRow} text-sm sm:text-base`}>
          <div className={`${LIST_STYLES.itemTagsLeft} tags-container-mobile`}>
            {reparacion.tipo && (
              <span
                className={`${LIST_STYLES.itemTagCategory} ${getColorFromString(reparacion.tipo, 'tipo')}`}
                title={reparacion.tipo}
              >
                <svg
            ...
            {reparacion.prioridad && (
              <span
                className={`${LIST_STYLES.itemTag} ${getPrioridadBadge(reparacion.prioridad)} text-sm sm:text-base px-3 py-1`}
              >
                <svg
                  className={ICON_STYLES.small}
                  fill="none"
            ...
            {fechaStr && (
              <span
                className={`${LIST_STYLES.itemTag} bg-gray-100 text-gray-700 text-sm sm:text-base px-3 py-1`}
              >
                <svg
                  className={ICON_STYLES.small}
                  fill="none"
          ...
          <div className={`${LIST_STYLES.itemTagsRight} text-sm sm:text-base`}>
            {typeof reparacion.costo_estimado === 'number' && (
              <span
                className={`${LIST_STYLES.itemTag} bg-brown-100 text-brown-800 text-sm sm:text-base px-3 py-1`}
              >
  ...
  const breadcrumbs = [{ label: 'Inicio', href: '/' }, { label: 'Reparaciones' }];
    ...
    >
      ...
      {(selectedReparacion || showAddModal) && (
        <ReparacionEditModal
          ...
          onSave={
            selectedReparacion?.id
              ? (data) => handleUpdateReparacion(selectedReparacion.id, data)
              : handleCreateReparacion
          }
