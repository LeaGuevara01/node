...
function MaquinariasPage({ token, onCreated }) {
  ...
  const fetchMaquinarias = async (filtrosActuales = {}, pagina = 1) => {
    ...
    try {
      console.log(
        'Fetching maquinarias with consolidated filters:',
        filtrosActuales,
        'page:',
        pagina
      );
      ...
      if (data.maquinarias) {
        ...
        actualizarPaginacion({
          current: 1,
          total: 1,
          totalItems: maquinariasOrdenadas.length,
          limit: 10,
        });
      }
      setError('');
  ...
  const renderMaquinaria = (maquinaria) => (
    <>
      ...
      {maquinaria.modelo && <div className={LIST_STYLES.itemDescription}>{maquinaria.modelo}</div>}
      <div className={LIST_STYLES.itemTagsRow}>
        <div className={`${LIST_STYLES.itemTagsLeft} tags-container-mobile`}>
          ...
          {maquinaria.anio && (
            <span className={`${LIST_STYLES.itemTag} bg-gray-100 text-gray-700 text-center`}>
