  ...
  const fetchProveedores = async (filtrosActuales = filtros, pagina = 1) => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:4000/api'}/proveedores?${params}`,
        {
  ...
  const fetchOpcionesFiltros = async () => {
    try {
      ...
      if (proveedoresData && proveedoresData.length > 0) {
        ...
        const ciudades = [
          ...new Set(
            proveedoresData.map((p) => extractCiudadFromDireccion(p.direccion)).filter(Boolean)
          ),
        ];
                    ...
                    <div className="flex-1">
                      ...
                      <div className={LIST_STYLES.itemTagsRow}>
                        <div className={`${LIST_STYLES.itemTagsLeft} tags-container-mobile`}>
                          {proveedor.cuit && (
                            <span
                              className={`${LIST_STYLES.itemTagCode} bg-brand-100 text-brand-800 hidden sm:flex`}
                              title={formatCuit(proveedor.cuit)}
                            >
                              <svg
                                className={ICON_STYLES.small}
                                fill="none"
                          ...
                          {proveedor.telefono && (
                            <span
                              className={`${LIST_STYLES.itemTag} bg-brown-100 text-brown-800`}
                              title={formatTelefono(proveedor.telefono)}
                            >
                              <svg
                                className={ICON_STYLES.small}
                                fill="none"
                          ...
                          {proveedor.direccion && (
                            <span
                              className={`${LIST_STYLES.itemTagLocation} ${getCiudadColorClass(extractCiudadFromDireccion(proveedor.direccion))}`}
                              title={extractCiudadFromDireccion(proveedor.direccion)}
                            >
                              <svg
                                className={ICON_STYLES.small}
                                fill="none"
