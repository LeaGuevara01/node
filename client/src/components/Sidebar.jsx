...
function Sidebar({ active, setActive, isMobileMenuOpen = false, setIsMobileMenuOpen }) {
  ...
  return (
    <>
      ...
      <nav
        className={`fixed top-0 left-0 h-full w-64 bg-gray-900 text-white shadow-xl z-50 transition-transform duration-300 ${
      ...
      >
              ...
              <li key={s.key}>
                <button
                  className={`w-full text-left px-4 py-3 rounded-lg transition-all duration-200 hover:bg-gray-800 hover:shadow-md flex items-center group ${
                    active === s.key
                      ? 'bg-gray-700 font-semibold border-l-4 border-blue-400 text-white'
                      : 'text-gray-300'
                  }`}
        ...
        <div className="border-t border-gray-700 p-4">
          <div className="text-xs text-gray-500 text-center">Sistema Agrícola v2.0</div>
        </div>
      </nav>
    </>
  );
