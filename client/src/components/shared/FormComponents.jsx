...
export const FormHeader = ({
  ...
  <p className="text-base text-gray-600 mt-1">{subtitle}</p>
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
        ...
        {showAdd && (
          <button onClick={onAdd} className={BUTTON_STYLES.newItem}>
            <svg
              className={ICON_STYLES.small}
              fill="none"
              stroke="currentColor"
...
export const FilterSection = ({ children, title = 'Filtros', onClearFilters }) => (
  ...
  <h2 className="text-base font-semibold text-gray-800 mb-2">{title}</h2>
    ...
    <div className={LAYOUT_STYLES.gridButtons}>
      <button type="button" onClick={onClearFilters} className={BUTTON_STYLES.filter.clear}>
...
export const SelectFilter = ({ value, onChange, options = [], placeholder, icon, field }) => (
  <div className="md:col-span-2 lg:col-span-1 xl:col-span-1">
    <div className={POSITION_STYLES.relative}>
      <div className={POSITION_STYLES.iconLeft}>{icon}</div>
      <select
        value={value}
...
export const EmptyState = ({ message = 'No hay elementos disponibles' }) => (
  <div className={LIST_STYLES.emptyState}>{message}</div>
);
