...
/**
 * Componente para mostrar iconos de estado de maquinaria
 * @param {string} estado - Estado de la maquinaria
 * @param {string} className - Clases CSS adicionales
 * @returns {JSX.Element} - Elemento SVG del icono
 */
function EstadoIcon({ estado, className = 'w-4 h-4' }) {
  if (!estado) {
    return (
  ...
  if (
    ...
    estadoLower.includes('reparacion') ||
  ...
  if (
    estadoLower.includes('averiado') ||
    estadoLower.includes('roto') ||
    estadoLower.includes('fuera de servicio')
  ) {
    return (
      <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path
