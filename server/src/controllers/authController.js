...
function issueDevToken(res) {
  const token = jwt.sign({ userId: 0, role: 'Admin', dev: true }, process.env.JWT_SECRET, {
    expiresIn: '1d',
  });
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    const allowFallback =
      String(process.env.ALLOW_DEV_FALLBACK || 'true').toLowerCase() !== 'false';
    const unreachable =
      err && (err.code === 'P1001' || /Can't reach database server/i.test(err.message));
