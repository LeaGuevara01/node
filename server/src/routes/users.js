...
const { createUser, getUsers, updateUser, deleteUser, getUserFilters } = require('../controllers/userController');

router.get('/', auth, getUsers);
router.post('/', auth, (req, res, next) => {
	if (!req.isAdmin) return res.status(403).json({ error: 'Solo administradores pueden crear usuarios' });
	next();
}, createUser);
...
router.post('/bulk', auth, upload.single('file'), async (req, res) => {
	try {
		if (!req.isAdmin) return res.status(403).json({ error: 'Solo administradores pueden importar usuarios' });
